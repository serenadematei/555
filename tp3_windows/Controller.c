...
int controller_loadFromText(char* path , LinkedList* pArrayListEmployee)
{
	...
	if(path!=NULL && pArrayListEmployee!=NULL){
		...
		{
			printf("Ocurrio un problema al abrir el archivo de texto.\n");
			exit(1);
		}
	...
	if(path!=NULL && pArrayListEmployee!=NULL){
		...
		for(int i=0; i<ll_len(pArrayListEmployee); i++){
			...
			if(auxEmpleado!=NULL &&
				...
				cant=fprintf(pFile,"%d    %s    %d    %d",id,nombre,horasTrabajadas,sueldo);
				if(cant==4){
					break;
				}
			}
